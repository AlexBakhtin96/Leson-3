...
* *git branch* - Выводит список веток в репозитории. (Звездочкой отмечается текущая ветка)

* *git branch_new_name* - Cоздает новую ветку с именем *new_name*

* *git branch -d new_name* - удаляет ветку с именем *new_name*.

* *git checkout branch_name* _ Позволяет перейти на ветку  с именем *branch name*.
